...
TetrisGameModel::TetrisGameModel()
    : m_gBoard(new TetrisBoardModel(10, 20)),
...
bool TetrisGameModel::genRandomShape(Tetromino *shape) {
  ...
  *(shape) = TetrominoFactory::randomTetromino();
  ...
  if (!checkPos(shape)) collision = true;
  return collision;
}
...
void TetrisGameModel::setGameControl(TetrisGameModel::gameControl control) {
  input = control;
}
